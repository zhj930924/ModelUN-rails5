  ...
  def crisis_updates
    if user_signed_in?
      if current_user.type == "Delegate"
        @cs_feed = CrisisUpdate.with_committees(current_user.committee).with_public(true).paginate(page: params[:cs_page])

      elsif current_user[:type] == "Crisis"
        @directive = current_user.crisis_updates.build
        @cs_feed = CrisisUpdate.with_committees(current_user.committee).paginate(page: params[:cs_page])
      end
